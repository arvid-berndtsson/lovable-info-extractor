export async function scrapeSecurityCenterProjectTable(tabId) {
    ...
    func: async () => {
      ...
      function readOverviewStatsCards() {
        ...
        for (const card of cards) {
          const labelNode = card.querySelector("span.text-sm");
          const valueNode = card.querySelector("span.text-2xl");
          const label = readText(labelNode).toLowerCase();
          const value = readCount(valueNode);
          if (!label) {
            continue;
          }
      ...
      function captureRows() {
        ...
        for (const row of rows) {
          ...
          const summary = {
            projectName,
            visibility: readText(cells[0]),
            errors: readCount(cells[1]),
            warnings: readCount(cells[2]),
            info: readCount(cells[3]),
            lastScan: readText(cells[4]),
            lastEdited: readText(cells[5]),
            securityViewHref: href
          };

          const key = summary.securityViewHref || `${summary.projectName}|${summary.lastEdited}`;
          if (!rowsByKey.has(key)) {
            added += 1;
          }

          rowsByKey.set(key, summary);
        }

        return added;
      }

      async function processScrollableTableOnCurrentPage() {
        ...
        for (let i = 0; i < 500; i += 1) {
          ...
          await sleep(120);
          const added = captureRows();

          const atBottom = maxScrollTop - viewport.scrollTop < 2;
          if (added === 0 && atBottom && Math.abs(previousTop - nextTop) < 2) {
      ...
      for (let pageLoop = 0; pageLoop < 100; pageLoop += 1) {
        pagesVisited += 1;
        await processScrollableTableOnCurrentPage();

        if (expectedTotalProjects && rowsByKey.size >= expectedTotalProjects) {
          break;
        }
      ...
      const viewLinks = rows.map((row) => row.securityViewHref).filter(Boolean);
